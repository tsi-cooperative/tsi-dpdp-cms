...
async function invokeBackendConsentAPI(preferences, mechanism) {
    ...
    const userId = localStorage.getItem('anon_user_id') || `anon_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;
    localStorage.setItem('anon_user_id', userId); // Persist anon ID if generated
...
/**
 * Links a previously anonymous client-side consent ID to an authenticated Data Principal ID.
 * This function should be called after a successful user login or registration.
 *
 * @param {string} authenticatedDataPrincipalId - The definitive, server-generated ID of the Data Principal (user_id).
 * @param {string} backendApiUrl - The base URL of your DPDP Solution backend API (e.g., 'http://localhost:3000/api/v1').
 */
async function linkConsentToDataPrincipal(authenticatedDataPrincipalId, backendApiUrl) {
    const anonymousUserId = localStorage.getItem("anon_user_id");
    const storedConsentData = localStorage.getItem(CONSENT_LOCAL_STORAGE_KEY);

    // Only proceed if there's an anonymous ID and stored consent to link
    if (anonymousUserId && storedConsentData && anonymousUserId !== authenticatedDataPrincipalId) {
        try {
            const consentPayload = {
                anonymous_user_id: anonymousUserId,
                authenticated_user_id: authenticatedDataPrincipalId,
                // You might also want to send the latest consent preferences from local storage
                // to ensure the backend has the most up-to-date state for this authenticated user.
                // However, the primary goal here is to link the ID itself.
                // The backend should then update all consent records associated with anonymous_user_id
                // to authenticated_user_id.
            };

            const response = await fetch(`${backendApiUrl}/consent/link-user`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    // If this linking API requires authentication (e.g., API Key or JWT),
                    // include it here. It's good practice for sensitive operations.
                    // 'X-API-KEY': 'YOUR_TSI_API_KEY_FOR_LINKING'
            ...
            if (response.ok) {
                console.log(`Consent records successfully linked for anonymous ID "${anonymousUserId}" to Data Principal ID "${authenticatedDataPrincipalId}".`);
                // IMPORTANT: Clear the anonymous ID from local storage AFTER successful linking
                // to prevent re-linking or using the old ID.
                localStorage.removeItem(ANONYMOUS_USER_ID_KEY);
                // The backend would ensure all existing consent records associated with
                // the anonymous ID are now associated with the authenticated ID.
                // Optionally, update the local storage consent record to reflect the new user_id
                const parsedConsent = JSON.parse(storedConsentData);
                parsedConsent.preferences.user_id = authenticatedDataPrincipalId; // Update user_id in the stored preferences
                localStorage.setItem(CONSENT_LOCAL_STORAGE_KEY, JSON.stringify(parsedConsent));

            } else {
                const errorData = await response.json();
                console.error(`Failed to link consent records: ${response.status}`, errorData);
            }
        } catch (error) {
            console.error('Network error during consent linking:', error);
        }
    } else if (anonymousUserId === authenticatedDataPrincipalId) {
        console.log('User already authenticated with the same ID, or no anonymous ID to link.');
        // Ensure the locally stored anonymous ID is cleaned up if it was a temporary one
        // but now matches the authenticated ID. This step is more of a cleanup.
        if (anonymousUserId.startsWith('anon_')) {
             localStorage.removeItem(ANONYMOUS_USER_ID_KEY);
        }
    } else {
        console.log('No anonymous consent data or ID found to link.');
    }
}

// --- How to use this function ---

// Example 1: After a user logs in successfully
// Assuming your login success callback provides the user's ID
// const loggedInUserId = 'server_generated_user_ABC123'; // This comes from your server after successful login
// linkConsentToDataPrincipal(loggedInUserId, 'http://localhost:3000/api/v1');


// Example 2: After a user registers and is automatically logged in
// const registeredUserId = 'server_generated_user_XYZ456'; // This comes from your server after successful registration
// linkConsentToDataPrincipal(registeredUserId, 'http://localhost:3000/api/v1');

// Example 3: When your app initializes and detects an authenticated user
// (e.g., if user is already logged in from a previous session)
// const authenticatedUserId = getAuthenticatedUserIdFromSession(); // Your function to get the current user ID
// if (authenticatedUserId) {
//     linkConsentToDataPrincipal(authenticatedUserId, 'http://localhost:3000/api/v1');
// }
